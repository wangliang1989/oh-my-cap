...
sub ps_arr {
   ...
   open(TRAV,"| trav > junk.$src_layer.$src.p") or die "couldn't run trav\n";
   ...
   close(TRAV);
   open(TRAV,"| trav > junk.$src_layer.$src.s") or die "couldn't run trav\n";
   printf TRAV "%d %d %d\n", $num_layer,$src_layer,$rcv_layer;
   for ($j=0;$j<$num_layer;$j++) {
      printf TRAV "%11.4f %11.4f\n",$th[$j],$vs[$j];
   }
   ...
   open(TRAV,"paste junk.$src_layer.$src.p junk.$src_layer.$src.s |");
   $j=0;
   while (<TRAV>) {
     @aaa = split;
     $tp = $aaa[1];
